...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                 ...
                 */
                /*
                 * EMPTY, DELIBERATELY. The application directory is in git and
                 * the served media with it; archiving it nightly on shared
                 * hosting would take minutes, eat the disk quota and produce
                 * files nobody restores. The database below is the part that
                 * cannot be recreated from anywhere else.
                 */
                'include' => [],
    ...
    |--------------------------------------------------------------------------
    |
    | The three success notifications are off deliberately.
    |
    ...
    | same sender, same subject line shape — is filtered with it. An alert that
    | only ever fires when something is wrong is an alert somebody still reads
    | in a year.
    |
    | The compensating check is the health route: /up reports the age of the
    | newest dump, so "no email" is verifiable rather than merely assumed.
    */
    'notifications' => [
        'notifications' => [
        ...
        'mail' => [
            'to' => env('BACKUP_ALERT_EMAIL', env('CLINIC_CONTACT_EMAIL', 'info@rehletsehha.com')),
    ...
    'monitor_backups' => [
        [
            ...
            'disks' => ['local'],
            'health_checks' => [
                // One day, because the schedule is nightly. Two days would
                // mean a cron that stopped on Monday is only noticed on
                // Wednesday.
                MaximumAgeInDays::class => 1,
                MaximumStorageInMegabytes::class => 500,
            ],
        ],
    ...
    'cleanup' => [
        ...
        'default_strategy' => [
             ...
             */
            /*
             * RETENTION, SIZED FOR SHARED HOSTING AND FOR HOW MISTAKES ARE
             * ACTUALLY FOUND.
             *
             * Seven days of everything covers the common case: somebody
             * deletes the wrong patient on Friday and nobody notices until
             * Monday. Sixteen daily, eight weekly and four monthly cover the
             * slower one — a data problem introduced weeks ago and only
             * visible now.
             *
             * The cap below is what keeps this from filling the account: a
             * dump of this database is small, and 500 MB is far more headroom
             * than the retention above can use.
             */
